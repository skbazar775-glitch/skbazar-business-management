...
<div class="container-fluid px-4 py-6">
    ...
    <div class="card bg-white shadow-lg rounded-xl overflow-hidden" data-aos="fade-up">
        <div class="card-header bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex justify-between items-center py-4 px-6">
            <h4 class="text-xl font-bold mb-0">Technicians</h4>
            <a href="{{ route('admin.technicians.create') }}" 
               class="btn bg-gradient-to-r from-teal-500 to-cyan-600 text-white px-4 py-2 rounded-lg hover:shadow-lg hover:scale-105 transition-all">
                <i class="fas fa-plus mr-2"></i> Add New Technician
            </a>
        </div>
        <div class="card-body p-6">
                ...
                <table class="w-full table-auto bg-white shadow-md rounded-lg">
                    ...
                    <tbody>
                        @forelse ($technicians as $technician)
                            <tr class="hover:bg-blue-50 hover:translate-x-1 transition-all" 
                                data-aos="fade-up" 
                                data-aos-delay="{{ 100 * $loop->index }}">
                                <td class="px-4 py-3">{{ $technician->id }}</td>
                                <td class="px-4 py-3 font-semibold">{{ $technician->name }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-3 text-center text-gray-500">No technicians found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="mt-4">
                {{ $technicians->links('pagination::tailwind') }}
            </div>
        </div>
    </div>
</div>
