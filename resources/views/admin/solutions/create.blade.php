<!-- resources/views/admin/solutions/create.blade.php -->
@extends('layouts.admin')

@section('title', 'Create Solar Solution')

@section('content')
    <h1 class="text-2xl font-bold mb-6">Create New Solar Solution</h1>
    ...
    <form action="{{ route('admin.solutions.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <select name="color_class" id="color_class" class="w-full p-2 rounded bg-gray-800 text-white border border-gray-700">
                <option value="" {{ old('color_class') == '' ? 'selected' : '' }}>Default (Blue)</option>
                <option value="hover-glow" {{ old('color_class') == 'hover-glow' ? 'selected' : '' }}>Blue</option>
                <option value="hover-glow-green" {{ old('color_class') == 'hover-glow-green' ? 'selected' : '' }}>Green</option>
                <option value="hover-glow-yellow" {{ old('color_class') == 'hover-glow-yellow' ? 'selected' : '' }}>Yellow</option>
            </select>
        </div>
        <div class="mb-4">
        ...
        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 hover-glow">Create Solution</button>
