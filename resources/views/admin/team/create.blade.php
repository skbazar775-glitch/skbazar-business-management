...
@section('content')
    <h1 class="text-2xl font-bold mb-6">Create New Team Member</h1>
    ...
    <form action="{{ route('admin.team.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <textarea name="bio" id="bio" class="w-full p-2 rounded bg-gray-800 text-white border border-gray-700">{{ old('bio') }}</textarea>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="url" name="linkedin_url" id="linkedin_url" class="w-full p-2 rounded bg-gray-800 text-white border border-gray-700" value="{{ old('linkedin_url') }}">
        </div>
        <div class="mb-4">
            <label for="email" class="block text-gray-300">Email (Optional)</label>
            <input type="email" name="email" id="email" class="w-full p-2 rounded bg-gray-800 text-white border border-gray-700" value="{{ old('email') }}">
        </div>
        <div class="mb-4">
            ...
            <select name="color_class" id="color_class" class="w-full p-2 rounded bg-gray-800 text-white border border-gray-700">
                <option value="" {{ old('color_class') == '' ? 'selected' : '' }}>None</option>
                <option value="text-blue-400" {{ old('color_class') == 'text-blue-400' ? 'selected' : '' }}>Blue</option>
                <option value="text-green-400" {{ old('color_class') == 'text-green-400' ? 'selected' : '' }}>Green</option>
        ...
        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 hover-glow">Create Team Member</button>
